  ...
  return (
    <>
      ...
      <div className="w-full h-full flex justify-between items-center px-6 bg-black/90 backdrop-blur-3xl relative z-[110] border-t border-white/5 shadow-[0_-10px_50px_rgba(0,0,0,0.5)]">
              ...
              {isPlaying && (
                <div className="flex items-end gap-0.5 h-3 mb-1">
                  <motion.div
                    ...
                    className="w-0.5 bg-groovify-green rounded-full"
                  />
                  <motion.div
        ...
        {/* Global Sound Resilience Helper */}
        <AnimatePresence>
          {isPlaying && !isBuffering && (
            <motion.button
              ...
              exit={{ opacity: 0, y: 50 }}
              onClick={resumeAudio}
              className="fixed bottom-24 right-10 z-[200] px-4 py-2 bg-groovify-green text-black font-black text-xs rounded-full shadow-[0_0_30px_rgba(29,185,84,0.4)] flex items-center gap-2 hover:scale-105 active:scale-95 transition-all"
            >
              <Volume2 size={16} />
              SOUND ISSUES? TAP HERE
            </motion.button>
          )}
        </AnimatePresence>
      </div>
